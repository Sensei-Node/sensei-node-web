...
const NewBox = ({
	...
	return (
		<a className='new-container' target='_blank' href={link}>
			<h3>{title}</h3>
			<span className='date'>{isProtocol ? link : date}</span>
...
const News = ({ isProtocol, token, language }) => {
	...
	useEffect(() => {
		if (!isFetching && !isProtocol) {
			...
			axios
				.get(
					`https://us-central1-senseiweb-d1c41.cloudfunctions.net/api/news/${lang}`
				)
				.then((response) => {
					...
					news.sort(function (a, b) {
						...
						return DateTime.fromFormat(a.date, 'd.m.yy').diff(
							DateTime.fromFormat(b.date, 'd.m.yy')
						).milliseconds;
					});
					setCardsInfo(news);
				})
		...
		<div id='news' css={[Styles]}>
			...
			>
				<div className='news-container'>
					{cardsInfo &&
						cardsInfo.map((newdata, index) => (
							<NewBox key={index} isProtocol={isProtocol} {...newdata} />
						))}
				</div>
			</div>
		</div>
